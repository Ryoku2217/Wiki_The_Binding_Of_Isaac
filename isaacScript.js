...
// Definición de iconos logros

const iconosLogros = {
  "Isaac's Head": "icons/Isaac'sHead.webp",
  "Lost Baby": "icons/lost-baby.webp",
  "Isaac's Tears": "icons/isaac's-tears.webp",
  "Mom's Knife": "icons/mom's-knife.webp",
  "Missing Poster": "icons/missing-poster.webp",
  "The D20": "icons/the-d20.webp",
  "Lil' Chest": "icons/lil'-chest.webp",
  "D1": "icons/d1.webp",
  "Fart Baby": "icons/fart-baby.webp",
  "Cry Baby": "icons/cry-baby.webp",
  "D infinity": "icons/d-infinity.webp",
...
function alternarTainted() {
  ...
  setTimeout(() => {
    ...
    const logrosHTML = Object.entries(personaje.logros).map(([key, value]) => {
      const icon = iconosLogros[key] || "";
      if (icon) {
        console.log(`Icono para ${key}: ${icon}`);
      } else {
        console.log(`No hay icono para ${key}`);
        console.log(iconosLogros["Isaac's Head"]); // Debería mostrar el camino del icono correspondiente

      }
      return `
        <li>
          ${icon ? `<img src="${icon}" alt="${key}" class="logros-icons">` : ""}
          <strong>${capitalizar(key)} - ${value}</strong>
        </li>
      `;
    }).join("");
    
    document.getElementById("logros-contenido").innerHTML = logrosHTML;
